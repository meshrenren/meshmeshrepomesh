	...
	<div class = "view-particular">
		<div class="box box-info" v-loading = "pageLoading">
            <div class="box-header with-border">
              	<h3 class="box-title">View Particular Disbursement</h3>
            </div>
	        <div class = "box-body">
	        	<el-row :gutter = "20">
	        		<el-col :span = "6">
	        			<el-form :model="searchForm" :rules = "formRule" ref="searchForm" label-width="100px" class="demo-ruleForm" label-position = "top">
							...
							<el-button class = "auto-width pull-right" size = "mini"  type = "primary" @click = "getVoucher">Filter</el-button>
						</el-form>
	        		</el-col>
	        		<el-col :span="18">
...
export default {
    ...
    computed:{
        totalAmount(){
        	let credit = 0
        	let debit = 0
        	_forEach(this.voucherList, pn =>{
        		credit += pn.credit ? parseFloat(pn.credit) : 0
        		debit += pn.debit ? parseFloat(pn.debit) : 0
        	})
	...
	methods:{	
		getVoucher(){
			...
			this.$refs.searchForm.validate(valid => {
    			if (valid) {
		            ...
		            this.$API.Voucher.getVoucherParticular(form)
		            .then(result => {
		                let res = result.data
		...
		printForm(type){
    		...
    		let data = {
    			...
    			transaction : this.voucherList
    		}

			this.$API.General.printList(data, type, 'VoucherParticular')
			.then(result => {
				let res = result.data
				if(type == 'pdf'){
					this.exporter(type, 'Particular Disbursement', res)
				}
				else if(type == 'print'){
					this.winPrint(res.data, 'Particular Disbursement')
				}
			})
