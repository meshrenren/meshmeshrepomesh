					...
					<el-row :gutter = "20">
	        			<el-col :span="12">
						  	...
						  	<el-form-item label="Amount" prop = "amount">
						    	<el-input-number v-model="tdAccountDetails.amount" controls-position="right" :min="1"></el-input-number>
						  	</el-form-item>
...
export default {
    ...
    methods: {
    	...
    	productChange(val){
    		this.getRate(val, this.tdAccountDetails.term)
    	},
    	termChange(val){
    		this.getRate(this.tdAccountDetails.fk_td_product, val)
    	},
    	getRate(product_id, term_count){
    		this.tdAccountDetails.interest_rate = 0
    		if(product_id && term_count > 0){
				...
				if(product){
					let rate = product.ratetable.find(ci => {
						return Number(term_count) >= Number(ci.day_from) && Number(term_count) <=  Number(ci.day_to)
					})
					if(rate){
