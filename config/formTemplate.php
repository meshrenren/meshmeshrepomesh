<?php

return [
	'header_layout'  	=> '<table class = "no-border" width = "100%">
            <tr>
            	<td>
            		<img width = "100px" src = "/images/coop_logo.png" />
            	</td>
            	<td>
            		<p style="font-size: 12pt; font-weight: bold;">DILG XI EMPLOYEES MULTI-PURPOSE COOPERATIVE SYSTEMS<p>
            		<p style="font-size: 10pt;">DILG Regional Office, #58 McArthur Highway, Davao City </p>
            		<p style="font-size: 10pt;">Tel #: 297-27-01 / 297-26-06 / 297-2604 </p>
            	</tr>
        </table>',

	'savings_deposit' 	=> '<div style="text-align: center;"><span style="font-size: 8pt;"><img width = "20px" src = "/images/coop_logo.png" /> DILG XI EMPLOYEES MULTI-PURPOSE
COOPERATIVE 
</span></div>
<div style="text-align: center;"><span>[account_type]</span></div>
<div style="text-align: center;">&nbsp;</div>
<table style="width: 100%;">
	<tbody>
		<tr>
			<td><span>&nbsp;Account Number:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[account_number]</span></td>
			<td width = "10px"></td>
			<td><span>&nbsp;Date:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[transaction_date]</span></td>
		</tr>
		<tr>
			<td><span>&nbsp;Account Name:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[account_name]</span></td>
			<td width = "10px"></td>
			<td><span>&nbsp;Type:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[transaction_type]</span></td>
		</tr>
		<tr>
			<td><span>&nbsp;Amount:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[transaction_amount]</span></td>
		</tr>
		<tr>
			<td><span>&nbsp;Remarks:</span></td>
			<td><span>&nbsp;[transaction_remarks]</span></td>
		</tr>


		<tr>
			<td></td>
			<td style = "text-align: center;">
				<div style = "width 100%; border-bottom: 2px solid #000"></div><br>
				<span>Approved By</span>
			</td>
		</tr>
	</tbody>
</table>',

	'savings_withdraw' 	=> '<div style="text-align: center;"><span><img width = "20px" src = "/images/coop_logo.png" /> DILG XI EMPLOYEES MULTI-PURPOSE
COOPERATIVE 
</span></div>
<div style="text-align: center;"><span>[account_type]</span></div>
<div style="text-align: center;">&nbsp;</div>
<table style="width: 50%;">
	<tbody>
		<tr>
			<td><span>Account Name:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[account_name]</span></td>
			<td width = "10px"></td>
		</tr>
		<tr>
			<td><span>Account No.:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[account_number]</span></td>
			<td width = "10px"></td>
		</tr>
		<tr>
			<td><span>Last Deposit/Withdraw:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[last_transaction]</span></td>
		</tr>
		<tr>
			<td><span>Balance:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[balance]</span></td>
		</tr>
		<tr>
			<td><span>Desired Withdrawal Amount:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[amount]</span></td>
		</tr>

		<tr>
			<td colspan = "2"  style = "padding-top : 40px;"></td>
		</td>

		<tr style = margin-top = "40px;">
			<td></td>
			<td style = "text-align: center;">
				<span>CHARITO L. CINCO</span>
				<div style = "border-bottom: 1px solid #000; width = "100%"></div>
				<span>Approved By</span>
			</td>
		</tr>
	</tbody>
</table>',

	'account_balance' 	=> '<div style="text-align: center;"><span><img width = "30px" src = "/images/coop_logo.png" /> DILG XI EMPLOYEES MULTI-PURPOSE
COOPERATIVE 
</span></div>
<div style="text-align: center;"><span>[account_type]</span></div>
<div style="text-align: center;">&nbsp;</div>
<table style="width: 50%;">
	<tbody>
		<tr>
			<td><span>Account Name:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[account_name]</span></td>
			<td width = "10px"></td>
		</tr>
		<tr>
			<td><span>Account No.:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[account_number]</span></td>
			<td width = "10px"></td>
		</tr>
		<tr>
			<td><span>Last Transaction:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[last_transaction]</span></td>
		</tr>
		<tr>
			<td><span>Balance:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[balance]</span></td>
		</tr>
	</tbody>
</table>',

	'timedeposit_certificate' 	=> '<div style = "text-align: center;"><p style="font-size: 12pt; font-weight: bold;">DILG XI EMPLOYEES MULTI-PURPOSE COOPERATIVE <br>
		<p style="font-size: 10pt;">DILG Regional Office, #58 McArthur Highway, Davao City </p>
		<p style = "font-size:18px">CERTIFICATE OF TIME DEPOSIT</p></div>
<table style="width: 100%; font-size: 9pt;">
	<tbody>
		<tr>
			<td width = "100px"><span style="font-size: 9pt;">Date Open:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[date_open]</span></td>
			<td width = "100px"></td>
			<td><span>Account No.:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[account_number]</span></td>
		</tr>
		<tr>
			<td width = "100px"><span style="font-size: 8pt;">Date Mature:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[date_mature]</span></td>
			<td width = "100px"></td>
			<td width = "100px"><span>OR Number:</span></td>
			<td style = "border-bottom: 1px solid #000;"><span>&nbsp;[or_number]</span></td>
		</tr>
	</tbody>
</table>
<div style = "margin-top: 40px;">This certifies that <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> of <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> has Time Deposit for <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> ( <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> ) days in the sum of <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> (P <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> ) Pesos bearing an interest of <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> ( <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> ) persent. per annum subject to the conditions stipulated by the Board of Directors. <div>'
]
?>